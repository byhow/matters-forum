import { db, users, curations } from "@/lib/db";
...
const getFeed = async function (idParam: string) {
  return (
    await db
      ...
      .from(curations)
      .where(sql`${curations.id} = ${idParam}`)
      .limit(1)
  )[0]; // .leftJoin(usersTable, sql`${usersTable.id} = ${curations.submitted_by}`)
};
...
export default async function ItemPage({
  ...
  const story = await getFeed(idParam);
